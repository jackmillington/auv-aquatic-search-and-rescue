...
#define SubIsHome(sub_x, sub_y) (sub_x == 1 && sub_y == 0)
...
std::string patCollect = packageDir + "/pat/collect_survivors.csp";
std::string patOutput = packageDir + "/pat/pat_output.txt";
std::string patWorld = packageDir + "/pat/world.csp";

std::string PAT_CMD_GO_HOME_BFS = "timeout " + std::to_string(MAX_BFS_TIME) + "s mono " + patExe + " -engine 1 " + patHome + " " + patOutput;
std::string PAT_CMD_GO_HOME_DFS = "mono " + patExe + " -engine 1 " + patHome + " " + patOutput;
std::string PAT_CMD_COLLECT_SURVIVORS_BFS = "timeout " + std::to_string(MAX_BFS_TIME) + "s mono " + patExe + " -engine 1 " + patCollect + " " + patOutput;
std::string PAT_CMD_COLLECT_SURVIVORS_DFS = "mono " + patExe + " " + patCollect + " " + patOutput;
...
void detect_hostiles(auv_search_rescue::Sensor &hostile_srv, int (&curr_world)[BOARD_H][BOARD_W], int &sub_x, int &sub_y);
...
int detect_survivors(auv_search_rescue::Sensor &survivor_srv, int (&curr_world)[BOARD_H][BOARD_W], int &sub_x, int &sub_y);
...
bool loadKnownWorld(const std::string &path,
	int (&world)[BOARD_H][BOARD_W])
{
	std::ifstream in(path);
	if (!in.is_open()) {
		ROS_ERROR("Couldn’t open %s", path.c_str());
		return false;
	}

	std::string line;
	// 1) skip all the #define’s until we hit the “var world…” line
	while (std::getline(in, line)) {
		if (line.find("var world") != std::string::npos) break;
	}

	// 2) now read exactly BOARD_H lines of comma‐separated ints
	for (int i = 0; i < BOARD_H; i++) {
		if (!std::getline(in, line)) {
			ROS_ERROR("world.csp truncated");
			return false;
		}
		std::stringstream ss(line);
		for (int j = 0; j < BOARD_W; j++) {
			int v;
			ss >> v;                     // parse the integer
			world[i][j] = v;
			if (j + 1 < BOARD_W) {
				char comma;
				ss >> comma;               // eat the ‘,’ or ‘;’
...
int main(int argc, char *argv[])
{
	...
	ros::init(argc, argv, "search_rescue_rescue");
	ros::NodeHandle n;
	ros::ServiceClient gridClient = n.serviceClient<auv_search_rescue::UpdateGrid>("/update_grid");
	ros::ServiceClient hostileSensorClient = n.serviceClient<auv_search_rescue::Sensor>("/hostile_sensor");
	ros::ServiceClient survivorSensorClient = n.serviceClient<auv_search_rescue::Sensor>("/survivor_sensor");
	auv_search_rescue::UpdateGrid grid_srv;
	auv_search_rescue::Sensor hostile_srv;
	auv_search_rescue::Sensor survivor_srv;
	...
	current_world[1][0] = SUB2;
	int sub_x = 1;
	int sub_y = 0;
	int currentPath = COLLECT_SURVIVORS;
  
	if (!loadKnownWorld(patWorld, true_world)) {
		ROS_ERROR("Failed to import initial world for sub2");
		return EXIT_FAILURE;
	}

	// now push it into Gazebo exactly as you did in sub1:
	auto grid_msg = createGrid(true_world);
		grid_srv.request.grid = grid_msg;
		if (!gridClient.call(grid_srv)) {
		ROS_ERROR("Couldn’t publish imported world to Gazebo");
		return EXIT_FAILURE;
	}

	if (!loadKnownWorld(patWorld, current_world)) {
		ROS_ERROR("Failed to import initial world for sub2");
		return EXIT_FAILURE;
	}
...
void update_directions(std::queue<std::string> &q)
{
	...
	std::ifstream pat_output(patOutput);
...
void generate_known_world(int (&world)[BOARD_H][BOARD_W], int &sub_x, int &sub_y, int &onBoard)
{
	...
	file << "\nvar world[Rows][Cols]:{Visited..Sub2} = [\n";
	for (int i = 0; i < BOARD_H; i++)
	{
		for (int j = 0; j < BOARD_W; j++)
		{

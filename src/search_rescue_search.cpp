...
#define patExe homeDir + "/Desktop/MONO-PAT-v3.6.0/PAT3.Console.exe"
// path to the explore path csp file for PAT
#define patExplore homeDir + "/catkin_ws/src/3806ict_auv_search_rescue/pat/explore.csp"
// path to the return home path csp file for PAT
#define patHome homeDir + "/catkin_ws/src/3806ict_auv_search_rescue/pat/return_home_search.csp"
// path to the collect survivors path csp file for PAT
#define patCollect homeDir + "/catkin_ws/src/3806ict_auv_search_rescue/pat/collect_survivors.csp"
// path to the output file generated by PAT
#define patOutput homeDir + "/catkin_ws/src/3806ict_auv_search_rescue/pat/pat_output.txt"
// path to world file generated by robot before calling PAT
#define patWorld homeDir + "/catkin_ws/src/3806ict_auv_search_rescue/pat/world.csp"
// timeout duration for BFS searches
#define MAX_BFS_TIME 10
// cmd to execute PAT using the explore path
std::string PAT_CMD_EXPLORE = "mono " + patExe + " " + patExplore + " " + patOutput;
...
int main(int argc, char *argv[])
{
	...
	while (ros::ok())
	{
		...
		if (SubIsHome(sub_x, sub_y) && survivors_seen == SURVIVOR_COUNT)
		{ // sub is at home position and has survivors on board
			ROS_INFO("All survivors located and sub is home. Mission complete!");
			return EXIT_SUCCESS;
		}
		...
		if (q.empty())
		{
			...
			if (survivors_seen == SURVIVOR_COUNT){
				currentPath = GO_HOME; // need a strategy to save those people
			}
		...
		if (survivors_seen == SURVIVOR_COUNT) {
			ROS_INFO("All survivors located — switching to return_home plan.");
			currentPath = GO_HOME;
			regenerate_moves(current_world, sub_x, sub_y, OnBoard, q, currentPath);
			// fall through to consume that full return‐home queue
		}
...
void generate_world(int (&world)[BOARD_H][BOARD_W])
{
	...
	world[SUB_START_X + 1][SUB_START_Y] = SUB2;
